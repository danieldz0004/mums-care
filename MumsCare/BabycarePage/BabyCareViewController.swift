...
class BabyCareViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        babyView.backgroundColor = UIColor(patternImage: #imageLiteral(resourceName: "background"))
    ...
    @IBAction func findCentre(_ sender: Any) {
        self.currentType = "Childcare"
    ...
    @IBAction func findKinder(_ sender: Any) {
        self.currentType = "Kindergarten"
        weak var weakSelf = self
        let alertvc = UIAlertController.init(title: NSLocalizedString("Please Enter a Postcode", comment: ""), message: nil, preferredStyle: .alert)
        let alertAction0 = UIAlertAction.init(title: NSLocalizedString("Confirm", comment: ""), style: .default) { (action) in
        ...
        workingGroup.notify(queue: workingQueue) {
            DispatchQueue.main.async(execute: {
                if placeidArray.count == 0 {
                    ...
                    ProgressHUD().showWithContent(NSLocalizedString("Sorry, there are no records. Please enter another postcode", comment: ""))
                    return
                }
        ...
        workingGroup.notify(queue: workingQueue) {
            DispatchQueue.main.async(execute: {
                ...
                if weakself?.tableViewDataDArray.count == 0 {
                    ProgressHUD().showWithContent(NSLocalizedString("No Rating Data For this Postcode", comment: ""))
                    return
                }
            })
