...
//  mom's care
//
//  Created by Daniel Dz on 3/4/19.
...
class PlacePreviewView: UIView {
    ...
    func setData(title: String, img: UIImage, rate: Int) {
        lblTitle.text = title
        imgView.image = img
        lblRate.text = "\(rate)"
    }
    ...
    func setupViews() {
        ...
        addSubview(lblRate)
        lblRate.centerXAnchor.constraint(equalTo: centerXAnchor).isActive=true
        lblRate.centerYAnchor.constraint(equalTo: imgView.centerYAnchor).isActive=true
        lblRate.widthAnchor.constraint(equalToConstant: 90).isActive=true
        lblRate.heightAnchor.constraint(equalToConstant: 40).isActive=true
    }
    ...
    let imgView: UIImageView = {
        let v=UIImageView()
        v.image=#imageLiteral(resourceName: "img")
        v.translatesAutoresizingMaskIntoConstraints=false
        return v
    }()
